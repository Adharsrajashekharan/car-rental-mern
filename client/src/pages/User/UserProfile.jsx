...
import commentImg from "../../assets/UserAssets/all-images/ava-1.jpg";
...
const UserProfile = () => {
  ...
  useEffect(() => {
    const fetchUserProfile = async () => {
      try {
        const res = await config.get('/api/v1/user/profile');
        setUser(res.data);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await config.put('/api/v1/user/me', formData, {
        headers: {
  ...
  return (
    <div>
      <Header/>
      <form className='forrrm' onSubmit={handleSubmit}>

        <div>
        <h2 style={{textAlign:'center'}}>User Profile</h2>

          <label className='llabel'>Name</label>
        ...
        <button className='butttton' type="submit">Update</button>
      </form>
    </div>
  );
};
